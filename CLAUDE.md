...
- **The interview transcripts are Hindi and the index is English-centric** (`all-MiniLM-L6-v2`). English RAG queries do not retrieve them (measured 0/5; Hindi queries 5/5). Tab 2 is unaffected because Gemini is multilingual. Any fix means a multilingual embedder plus a full re-index, on both `preprocess.py` and `rag_engine.get_embedding_function()`.
